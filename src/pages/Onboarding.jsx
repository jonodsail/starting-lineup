...
export default function Onboarding() {
  ...
  const [form, setForm] = useState(profile || { name: member?.name || '', classYear: '', careerStage: '', functions: [], sectors: [], locations: [], resumeName: '' })
  ...
  return <div className="min-h-screen bg-canvas px-5 py-8 md:px-10 lg:px-14"><div className="mx-auto max-w-3xl">
    ...
    <section className="panel mt-8 p-6 md:p-10">
      ...
      <div className="mt-10 flex justify-between border-t border-line pt-6"><button disabled={step === 0} onClick={() => setStep(value => value - 1)} className="inline-flex items-center gap-2 text-sm font-semibold text-ink-muted disabled:invisible"><ArrowLeft size={16} />Back</button><button disabled={!canContinue} onClick={() => step === steps.length - 1 ? finish() : setStep(value => value + 1)} className="btn-primary disabled:cursor-not-allowed disabled:opacity-35">{step === steps.length - 1 ? 'Enter Starting Lineup' : 'Continue'}<ArrowRight size={16} /></button></div>
    </section>
  </div></div>
}
