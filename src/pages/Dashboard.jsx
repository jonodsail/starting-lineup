...
// The onboarding "Search stage" answer drives the recommended next step, so
// the question a member answers in orientation changes what they are asked to
// do. Falls back to the recruiting action when no stage was chosen.
const NEXT_STEP_BY_STAGE = {
  'Exploring sports': 'Read two roles in each sector you picked, and note which companies keep coming up.',
  'Actively recruiting': 'Save three target roles, then identify one alum at each company before outreach.',
  'Networking first': 'Pick one company on the board and message one alum there this week.',
  'Committed, staying connected': 'Scan this week’s new roles and pass one to a classmate who is still searching.',
}
const DEFAULT_NEXT_STEP = NEXT_STEP_BY_STAGE['Actively recruiting']
...
export default function Dashboard() {
  ...
  // Nothing matched, so the list below is the newest roles rather than a match.
  // Say so, instead of labelling them as matches.
  const showingFallback = !loading && opportunities.length > 0 && preferred.length === 0
  const featured = (preferred.length ? preferred : opportunities).slice(0, 3)
  const firstName = profile?.name?.split(' ')[0] || 'there'
  const nextStep = NEXT_STEP_BY_STAGE[profile?.careerStage] || DEFAULT_NEXT_STEP

  return <div className="page-wrap">
    <section className="relative overflow-hidden rounded-3xl bg-night px-6 py-8 text-white md:px-10 md:py-10"><div className="field-lines absolute inset-0 opacity-20" /><div className="relative flex flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-crimson-light">Home</p><h1 className="mt-3 font-display text-4xl font-bold tracking-tight md:text-6xl">{greeting()}, {firstName}.</h1><p className="mt-3 max-w-xl text-sm leading-6 text-white/55">Opportunities and people that can move your search forward.</p></div><Link to="/opportunities" className="btn-light">Explore all roles <ArrowRight size={16} /></Link></div></section>

    <div className="mt-6 grid gap-4 sm:grid-cols-3">
      <Stat value={loading ? '—' : opportunities.length} label="Curated opportunities" note="MBA-relevant roles only" />
      <Stat value={alumniCount === null ? '—' : alumniCount} label="Alumni profiles" note={isSupabaseConfigured ? (alumniCount === null ? 'Count unavailable' : 'Verified by club officers') : 'Preview'} />
    ...
    <div className="mt-10 flex items-center gap-2 rounded-xl border border-line bg-white px-4 py-3 text-xs text-ink-muted"><BriefcaseBusiness size={15} />Always confirm alumnus role on LinkedIn or company websites before reaching out.</div>
  </div>
}
