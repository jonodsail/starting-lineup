...
import { formatDomains } from '../lib/config'

export default function Login() {
  const { allowedDomains, member, authError, authNotice, signIn, enterPreview, isConfigured } = useAuth()
  ...
  return <div className="min-h-screen bg-night text-white lg:grid lg:grid-cols-[1.1fr_0.9fr]">
    <section className="relative flex min-h-[46vh] flex-col justify-between overflow-hidden px-7 py-8 lg:min-h-screen lg:px-14 lg:py-12">
      ...
      <div className="relative max-w-2xl py-14"><h1 className="mt-5 font-display text-5xl font-bold leading-[0.98] tracking-tight md:text-7xl">Your path into the business of sports starts here.</h1><p className="mt-6 max-w-xl text-base leading-7 text-white/60">Top MBA opportunities, alumni search, and trackers to manage your search.</p></div>
    ...
    <section className="flex items-center bg-canvas px-7 py-14 text-ink lg:px-16"><div className="mx-auto w-full max-w-md"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-crimson-soft text-crimson"><LockKeyhole size={22} /></div><h2 className="mt-6 font-display text-4xl font-bold text-night">Member access</h2><p className="mt-3 text-sm leading-6 text-ink-muted">Use your RC or EC HBS email. We’ll send a secure sign-in link to your Microsoft-managed inbox.</p>
      <form className="mt-8" onSubmit={handleSubmit}>
        ...
        <input id="hbs-email" className="input" type="email" autoComplete="email" inputMode="email" placeholder="yourname@mba202x.hbs.edu" value={email} onChange={(event) => setEmail(event.target.value)} disabled={sending} required />
        <p className="mt-2 text-xs leading-5 text-ink-muted">Access is limited to <strong className="font-semibold text-ink">{formatDomains(allowedDomains)}</strong>.</p>
      ...
      {!isConfigured && <p className="mt-5 text-xs leading-5 text-ink-muted">Sign-in is not available yet.</p>}
    </div></section>
  </div>
}
