...
export function OpportunityCard({ opportunity, saved, onSave }) {
  return <article className="panel group p-5 transition hover:-translate-y-0.5 hover:border-ink/20 hover:shadow-lg hover:shadow-night/5">
    ...
    <div className="mt-5 border-t border-line pt-4"><p className="text-xs font-medium text-ink"><span className="text-ink-muted">Why it made the cut:</span> {opportunity.mbaSignal}</p><p className="mt-2 text-[11px] text-ink-muted">Posting verified {new Date(`${opportunity.verifiedOn}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p><div className="mt-4 flex items-center justify-between"><button onClick={() => onSave(opportunity)} className={`inline-flex items-center gap-2 text-sm font-semibold ${saved ? 'text-forest' : 'text-ink hover:text-crimson'}`}>{saved ? <Check size={16} /> : <Bookmark size={16} />}{saved ? 'Saved' : 'Save role'}</button><a href={opportunity.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-semibold text-night hover:text-crimson">Open job posting <ArrowUpRight size={15} /></a></div></div>
  </article>
}
